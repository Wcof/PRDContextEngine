...
执行时必须依次完成上述步骤，不可跳步。步骤产出写入 `process/06-aiprd-delivery.md`。
...
- `docs/pm-context/process/06-aiprd-delivery.md`（交付物清单 + 追溯映射 + 审计三元组）
...
| **PMContext §8 有前端框架声明但 ai-prd 未承接为「技术栈契约」段** | 补写技术栈契约段，标注"补充：来自 §8 声明" | 不阻塞；下游 pm-sketch Step -0.5 此场景可命中，不影响原型生成 |
