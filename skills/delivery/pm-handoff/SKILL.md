---
name: pm-handoff
description: Use when the user asks to handoff or continue in another session, mentions 交接、handoff、切换会话、continue later、session handoff、换 agent、接续工作、上下文压缩、compact session、知识转移、knowledge transfer、条件触发、condition-based trigger.
metadata:
  internal: true
---
...
执行时必须依次完成上述步骤，不可跳步。步骤产出写入 `process/06-handoff-delivery.md`。
...
- `docs/pm-context/process/06-handoff-delivery.md`（状态盘点映射 + 审计三元组）
...
| 无法确定"停在哪一步" | 从最后修改的 `process/` 文件推断进度 | 无法推断则标注"进度不明，建议从 skill 起点重跑" |
