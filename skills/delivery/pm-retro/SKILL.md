...
PMContext + `process/` 中间工件 + 产物（PRD/OST/草图等）是回顾素材。本 skill 聚合这些产物与团队反馈，输出行动项并把经验回灌 PMContext。回顾是 PM Thinking Loop 的闭环步骤。

## Instructions

- [ ] PMContext 已读取（不存在则 STOP 提示运行 /pm-need）
- [ ] `process/` 与产物目录已扫描（迭代工件来源）
...
执行时必须依次完成上述步骤，不可跳步。步骤产出写入 `process/06-retro-delivery.md`。
...
- `docs/pm-context/process/*`（各 skill 中间工件，看流程瓶颈）
...
闭环步骤产出完成后，写入中间工件：
- `docs/pm-context/process/06-retro-delivery.md`（主题聚合+行动项+经验回灌 + 审计三元组）
...
| **🔴 STOP**：无 `process/` 工件且无团队反馈 | 提示"未发现迭代素材，提供反馈或确认迭代已开始" | 退出，不臆造回顾 |
...
| 跳过 `process/` 工件只看反馈 | 工件暴露流程瓶颈（哪步反复自愈），是回顾金矿 |
