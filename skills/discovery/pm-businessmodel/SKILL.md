...
执行时依次完成，步骤产出写入 `.loop/bm-step2.md`、`.loop/bm-step4.md`。
...
**自愈机制**：依赖检查失败时回溯重生成（最多 3 轮），超限降级为标 `[待确认]` + 信息缺口 + 终止并告知用户
...
**🔴 CHECKPOINT** — 输出产物路径 + 业务游戏 + 9 模块完整度 + 收入流数 + `[假设]` 项数。
...
| 审计三元组写"基于上述依据产出" | 空话，判定 Failure（ADR 0008 §11） |
