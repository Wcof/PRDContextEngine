---
name: pm-grill
description: Use when the user asks to stress-test or pressure-test PMContext, mentions 质询、压力测试、红队、red team、grill、盘问、挑刺、stress test、challenge assumptions、攻击假设、承重假设、kill criteria、最便宜测试、steelman.
metadata:
  internal: true
---
...
### Step 2.5: 分支穷举追问模式（relentless interview — 借鉴 skills/grilling 的 "Walk down each branch of the design tree"）

> 默认模式（Step 2）是全景扫描——一次性列出所有承重假设并攻击。但承重假设之间几乎总有依赖（假设 A 成立才假设 B 可成立），扫完一次给等于让 PM 在缺前置信息下瞎判。**追问模式**（本步）走决策树——每个依赖分支**穷举到底**，不止于"问一个就推"。

**触发条件**：承重假设之间存在依赖关系（显式或可推断即触发——不再要求"显式依赖链"才触发）。Agent 先扫承重假设集合，若其中任一假设的成立依赖另一假设的结论，即进入分支穷举追问模式。无依赖时才用 Step 2 全景扫描。

**建依赖树**（借鉴 pm-refine Step 0 的决策依赖树机制）：
1. 扫所有承重假设，判每条的"依赖前件"——该假设成立需要哪些其它假设先成立
2. 默认依赖骨架（材料覆盖时按材料覆写）：
   - 用户增长假设依赖定价假设（增长速率由定价调节）
   - 转化率假设依赖渠道假设（不同渠道转化差异大）
   - 留存假设依赖核心价值假设（留存是价值的回声）
   - 度量假设依赖用户行为假设（度量映射的是哪类用户的行为）
3. 依赖树内嵌到 `process/01-grill-deps-tree.md`，含拓扑序 + Mermaid 树状图

**分支穷举执行**（对齐 grilling 的 relentless 纪律）：
```
for each 承重假设分支（按依赖拓扑序）:
  1. 等其依赖前件全部被质询确认后才开始本分支
  2. 走本分支的每个子问题到底——不止问"假设成立吗"，连问"若成立，阈值多少""若不成立，替代假设是什么"
  3. 每问只问 1 个，附推荐答案（"我们建议认为 X，因为 PMContext 证据 Y + 前件分支已确认 Z"）
  4. 等 PM 反馈（多问一起=让人困惑）
     - PM 确认 → 采信，推进到本分支下一个子问题
     - PM 否认 → 重估假设，可能改依赖树下游分支路径
     - PM 不确定 → 标 `[待确认]` 继续，但标注下游分支风险受限
  5. 能查代码/数据/PMContext 原文的则不问（"不要问如果可以直接查"）
  6. 本分支所有子问题穷尽后才推下一分支
```

追问模式不走标准 Step 3-4，走完 Step 2.5 后直接进入 Step 5 排序。

**穷举纪律**（借鉴 grilling 的 "one at a time" + pm-refine Step 0 的拓扑遍历）：
- 一次只问 1 个问题，禁合并提问
- 每问附带推荐答案，禁让 PM 从零回答
- 可查的直接查，不制造无意义的问题
- **🔴 分支必须穷尽**：本分支子问题没问完不推下一分支（旧规则"走完显式链即可"会让依赖树中下游分支在前件未穷尽时被问，PM 被迫瞎判）
- 依赖分支走完即止，不走完全部分支也可退出（PM 说"停"按退出机制处理）
...
- 追问模式 Step 2.5 建树产出 → `docs/pm-context/process/01-grill-deps-tree.md`（承重假设依赖骨架 + 拓扑序 + Mermaid 树状图）
