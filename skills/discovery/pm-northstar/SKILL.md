---
name: pm-northstar
description: 从 PMContext 深化北极星指标——业务游戏分类（注意力/交易/生产力）+ 单一 NSM（七准则校验）+ 3-5 个 Input Metrics 星座 + 健康指标（guardrail）+ Mermaid 指标树 + 与 KPI/OKR 关系澄清。Use when the user asks for North Star metric deep dive, mentions 北极星指标深化、north star、NSM、input metric、指标星座、metrics constellation、健康指标、guardrail metric、OMTM、北极星框架、north star framework.
---
...
**自愈机制**：失败回溯重生成（最多 3 轮），超限降级标 `[待确认]` + 终止
