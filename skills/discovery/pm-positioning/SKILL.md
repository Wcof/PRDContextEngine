...
### Step 2.5: 定位创意发散（借鉴 pm-marketing-growth/positioning-ideas）

> 6 段式确定的"是什么"，定位创意解决"凭什么这么说"——在竞品占据的认知空间中找未占领的差异化角度。必须先发散 ≥3 个定位角度，再选最优进 Moore 模板。

从竞品层与用户场景提取差异化机会，发散不同定位角度：

| # | 定位角度 | 定位陈述草稿 | 差异化来源 | 竞品盲区 | 推荐? |
|---|---------|------------|-----------|---------|------|
| 1 | <角度名> | <For X, who Y, Z 是...> | PMContext 用户场景/竞品 | <竞品忽略的维度> | ⭐/备选 |
| 2 | ... | | | | |

**生成规则**：
- ≥3 个定位角度（少于 3 个发散不足）
- 每角度必须说明"为什么竞品不这么说"（竞品盲区），否则可能撞定位
- 每角度必须标注 PMContext 来源（差异化来源列）
- 选最优进 Step 3 Moore 模板，其余保留为备选

**反模式**：只写一个定位陈述——没有备选的定位是未经选择的假设。
```
